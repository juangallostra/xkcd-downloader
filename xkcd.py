...
class ComicInstance():
    ...
    def get_comic_data(self):
        ...
        """
        url = self._URL_init + self.comic_number + self._URL_end
    ...
    def download_image(self):
        """
        Retrieve the actual comic image from its url and name via urllib.request
        and save it.
        ...
        try:
            ...
            image = urllib.request.urlretrieve(self.image_url,
                                        self.comic_name)
            self._save_comic()
            return True
        except:
    ...
    def _save_comic(self):
        ...
        with Image.open(self.comic_name) as image:
            ...
            image.save(self._IMG_DIR + self.comic_name + "." +
                       self.image_url[-3:], self._IMG_EXTENSION[self.image_url[-3:]])
    ...
    def get_explanation(self):
        if self.txt_explanation is None:
            ...
            div = soup.find(
                'div', attrs={'id': 'mw-content-text', 'class': 'mw-content-ltr'})

            # Find all of the text between paragraph tags and strip out the html
            paragraph_text = [paragraph.getText()
                              for paragraph in div.find_all('p')]
            # make sure all characters are ascii encoded
            explanation_text = [text.encode(
                'ascii', 'ignore') for text in paragraph_text]
...
parser.add_argument('comics', metavar='N', type=str, nargs='*',
                    help='Comic numbers. Can be a combination of ranges: i-j and individual comics: i, or left blank which, when combined with the flag -g will download the latest comic')
parser.add_argument('-g', '--get', help='get comic images',
                    action='store_true')
parser.add_argument(
    '-s', '--show', help='show comic images', action='store_true')
parser.add_argument('-e', '--explain',
                    help='get comic explanations', action='store_true')
parser.add_argument(
    '-a', '--all', help='download all published comics', action='store_true')
parser.add_argument(
    '-r', '--random', help='download a random comic', action='store_true')
...
def pretty_print(comic):
    if platform.system() == 'Linux':
        ...
        print('-' * free_side_cols + comic.comic_name +
              '-' * free_side_cols + '\n')
...
def download_all_comics(args):
    ...
    while True:
        if index != 404:
            ...
            if downloaded:
                print(successful_download + str(index) + " out of " +
                      str(max_comic) + " (" + str(100 * float(index) / max_comic) + " %)")
...
def download_specific_comics(args):
    ...
    if any('-' in r_comic for r_comic in args.comics):
        ...
        comics = [range(int(i[0]), int(i[1]) + 1) if len(i) >
                  1 else int(i[0]) for i in comics_range]
        # Flatten list of lists to get all individual comic numbers to download
        flatten = lambda *args: (result for mid in args for result in (
            flatten(*mid) if isinstance(mid, list) else (mid,)))
